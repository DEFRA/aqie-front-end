...
class DAQICurrentImplementationTester {
  ...
  testDAQILevel(level, temporary = true) {
    ...
    segments.forEach((segment, index) => {
      ...
      // Remove any existing daqi-# classes AND daqi-selected
      segment.className = segment.className
        .replace(/daqi-\d+/g, '')
        .replace(/daqi-selected/g, '')

      if (segmentNumber === level) {
        // This is the active segment - apply ONLY the specific level class
  ...
  resetToDefault() {
    ...
    segments.forEach((segment) => {
      // Remove all daqi-# classes AND daqi-selected
      segment.className = segment.className
        .replace(/daqi-\d+/g, '')
        .replace(/daqi-selected/g, '')
      // Add default class
  ...
  async testAllLevels(delay = 2000) {
    ...
    for (let level = 1; level <= 10; level++) {
      ...
      // Double-check the color is applied correctly
      const computedStyle = window.getComputedStyle(activeSegment)
      const bgColor = computedStyle.backgroundColor

      console.log(`   ✅ Level ${level} applied`)
      console.log(`   🎨 Background: ${bgColor}`)
      console.log(`   📱 Classes: ${activeSegment.className}`)

      // Special validation for level 10 (purple)
      if (level === 10) {
        const isPurple =
          bgColor === 'rgb(76, 44, 146)' || bgColor.includes('76, 44, 146')
        console.log(
          `   💜 Purple check: ${isPurple ? '✅ CORRECT' : '❌ WRONG - Expected rgb(76, 44, 146)'}`
        )

        if (!isPurple) {
          console.warn('⚠️ Level 10 is not showing purple! Attempting fix...')
          // Try to fix it by reapplying
          activeSegment.classList.remove('daqi-0', 'daqi-10')
          activeSegment.offsetHeight // eslint-disable-line no-unused-expressions
          activeSegment.classList.add('daqi-10')
          activeSegment.offsetHeight // eslint-disable-line no-unused-expressions

          const newStyle = window.getComputedStyle(activeSegment)
          console.log(`   🔄 After fix: ${newStyle.backgroundColor}`)
        }
      }
  ...
  /**
   * Specialized test for Level 10 purple color issue
   */
  testLevel10Purple() {
    if (!this.container) {
      console.error('❌ Tester not initialized. Call init() first.')
    ...
    console.log('\n💜 TESTING LEVEL 10 PURPLE COLOR SPECIFICALLY')
    console.log('═'.repeat(60))

    const segments = this.container.querySelectorAll('.daqi-bar-segment')
    const segment10 = segments[9] // 10th segment (0-based index)
    ...
    // Step 1: Clean all classes
    console.log('\n2️⃣ Cleaning all DAQI classes...')
    ...
    // Step 2: Add daqi-10 class only
    console.log('\n3️⃣ Adding daqi-10 class...')
    ...
    // Step 3: Add daqi-selected class
    console.log('\n4️⃣ Adding daqi-selected class...')
    ...
    // Visual highlight for 3 seconds
    segment10.style.outline = '3px solid yellow'
    console.log(
      '\n👁️ Visual check: Segment 10 should now be purple with yellow outline'
    )

    setTimeout(() => {
      segment10.style.outline = ''
      // Restore original state
    ...
    return {
      finalBackground: style2.backgroundColor,
      isPurple: style2.backgroundColor === 'rgb(76, 44, 146)',
      finalClasses: segment10.className
    }
  }
...
window.testCurrentDAQI = {
  ...
  /**
   * Test Level 10 purple color specifically
   */
  testPurple: () => daqiTester.testLevel10Purple(),
  ...
  document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
      if (daqiTester.init()) {
        ...
        console.log(
          '  testCurrentDAQI.testPurple()     - Test level 10 purple specifically'
        )
        console.log(
