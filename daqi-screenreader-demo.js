...
(function() {
  ...
  function demoTabNavigation() {
    ...
    tabs.forEach((tab, index) => {
      ...
      console.log(`   🔊 Screen reader announces: "${ariaLabel || textContent}"`);
  ...
  function showScreenReaderTestingGuide() {
    ...
    console.log('• JAWS (Trial, Windows): https://www.freedomscientific.com/downloads/jaws/');
    ...
    console.log('2. Listen for: "Daily Air Quality Index X out of 10, [Level] pollution level"');
  ...
  function testDAQIValueAnnouncements() {
    ...
    const daqiLevels = [
      { values: [1, 2, 3], level: 'Low', color: 'Green', advice: 'Enjoy usual outdoor activities' },
      { values: [4, 5, 6], level: 'Moderate', color: 'Yellow', advice: 'Adults and children with lung problems, and adults with heart problems, who experience symptoms, should consider reducing strenuous physical activity' },
      { values: [7, 8, 9], level: 'High', color: 'Red', advice: 'Adults and children with lung problems, and adults with heart problems, should reduce strenuous physical exertion' },
      { values: [10], level: 'Very High', color: 'Purple', advice: 'Adults and children with lung problems, adults with heart problems, and older people, should avoid strenuous physical exertion' }
    ];
    
    daqiLevels.forEach(({ values, level, color, advice }) => {
      console.log(`\n📊 DAQI Level: ${level} (${color})`);
      console.log(`   Values: ${values.join(', ')}`);
      console.log(`   🔊 Screen reader should announce: "DAQI X, ${level} pollution level"`);
