...
const handleSingleMatch = (
  ...
  logger.info(`getForecasts?.forecastsxxxx2: ${JSON.stringify(getForecasts?.forecasts)}`)
...
const handleMultipleMatches = (
  ...
  logger.info(`getForecasts?.forecastsxxxx: ${JSON.stringify(getForecasts?.forecasts)}`)
  request.yar.set('locationData', {
    results: selectedMatches,
    getForecasts: getForecasts?.forecasts,
    multipleLocations,
