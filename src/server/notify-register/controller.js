...
const getMobilePhoneController = {
  handler: async (request, h) => {
    ...
    return h.view('notify-register/mobile-phone', {
      ...
      description: 'We will send you a text message with a 5-digit activation code.',
      page: 'Text alerts',
...
const postConfirmMobileController = {
  handler: async (request, h) => {
    try {
      ...
      const mobileNumber = request.yar?.get('mobileNumber') || request.payload?.mobilePhone || '07123456789'
      ...
      const smsResult = await notifyService.sendActivationCode(mobileNumber, activationCode)
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        request.yar?.set('mobileNumber', request.yar?.get('mobileNumber') || '07123456789')
        request.yar?.set('isDevelopmentFallback', true)
        
        console.log(`📱 FALLBACK MODE - Your activation code is: ${mockActivationCode}`)
        
        return h.redirect('/notify/activation-code').code(REDIRECT_STATUS_CODE)
      }
      
      // In production, show error page or redirect with error
      return h.view('notify-register/error', {
    ...
    try {
      ...
      if (codeAge > maxAge) {
        return h.view('notify-register/activation-code', {
          ...
          heading: 'Enter your activation code',
          description: `We sent a 5-digit code to ${request.yar?.get('mobileNumber') || '07123456789'}`,
          error: { text: 'Your activation code has expired. Please request a new one.' },
          page: 'Text alerts',
          serviceName: 'Check air quality',
          footerTxt: english.footerTxt,
          phaseBanner: english.phaseBanner,
  ...
  handler: async (request, h) => {
    try {
      ...
      if (!locationDetails || !locationDetails.id) {
        logger.warn('Attempted registration without location details in session')
        return h.view('notify-register/error', {
          pageTitle: 'Location not found',
          heading: 'Location details missing',
          description: 'Please search for and select a location before registering for alerts.',
          page: 'Text alerts',
          serviceName: 'Check air quality',
          footerTxt: english.footerTxt,
          phaseBanner: english.phaseBanner,
      ...
      const areaRegionName = locationDetails.area || locationDetails.region || ''
      ...
      const registrationResult = await notifyService.registerForAlerts(alertConfig)
      ...
      if (process.env.NODE_ENV === 'development') {
        logger.warn('Falling back to success page despite API error (development mode)')
        request.yar?.set('registrationComplete', true)
...
const getSuccessController = {
  handler: async (request, h) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      if (apiError) {
        developmentInfo = 'Note: Registration completed with API fallback (development mode)'
...
const getEmailAlertsController = {
  handler: async (request, h) => {
    ...
    return h.view('notify-register/email-alerts', {
      ...
      description: 'Email alerts for air quality are coming soon. Sign up for text alerts instead.',
      page: 'Text alerts',
      serviceName: 'Check air quality',
      footerTxt,
      phaseBanner,
