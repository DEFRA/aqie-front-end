  ...
  containers.forEach((container) => {
    ...
    const calculateDividers = (
      ...
      if (containerNeedsAdjustmentsParam) {
        ...
        if (containerWidthParam <= 430) {
          ...
          if (viewportWidth === 430) {
            // iPhone 14 Pro Max specific: move dividers left
            divider1Adjustment = -1 // Move another 1px left from 0
            divider2Adjustment = 0 // Move another 1px left from 1
            divider3Adjustment = 1 // Move another 1px left from 2
          } else {
            // Other narrow containers: move dividers further left for better alignment
            divider1Adjustment = -3 // Move another 1px left from -2
            divider2Adjustment = -3 // Move another 1px left from -2
            divider3Adjustment = -4 // Move another 1px left from -3
          }
        } else {
          // Medium narrow container: apply moderate adjustments moved another 1px left
          divider1Adjustment = -2 // Move another 1px left from -1
          divider2Adjustment = -3 // Move another 1px left from -2
          divider3Adjustment = -3 // Move another 1px left from -2
        }
      } else {
        // Wide container: move all dividers another 1px left
        divider1Adjustment = -2 // Move another 1px left from -1
        divider2Adjustment = -2 // Move another 1px left from -1
        divider3Adjustment = -2 // Move another 1px left from -1
      ...
      if (containerNeedsAdjustmentsParam) {
        ...
        if (viewportWidth === 430) {
          // iPhone 14 Pro Max adjustments applied (right positioning)
        } else {
          // Mobile layout adjustments applied (left positioning)
        }
      } else {
        // Desktop layout maintained - debug info removed for production
      }
    }
